...
# --- БАЗА ДАННЫХ (С сохранением прогресса навсегда при любых обновлениях) ---
def init_db():
...
# --- КАТАЛОГИ (ДОХОД УКАЗАН ЗА 1 МИНУТУ) ---
HOUSES = {
    "house_1": {"name": "🏠 Шалаш", "price": 20, "income": 0.6},
    "house_2": {"name": "🛖 Хижина", "price": 50, "income": 1.8},
    "house_3": {"name": "🏚️ Лачуга", "price": 120, "income": 4.8},
    "house_4": {"name": "⛺ Палаточный лагерь", "price": 300, "income": 12.0},
    "house_5": {"name": "🏠 Гараж", "price": 700, "income": 30.0},
    "house_6": {"name": "🏡 Дачный домик", "price": 1500, "income": 72.0},
    "house_7": {"name": "🏘️ Коттедж", "price": 3500, "income": 180.0},
    "house_8": {"name": "🏢 Офис", "price": 8000, "income": 450.0},
    "house_9": {"name": "🏬 Торговая точка", "price": 18000, "income": 1080.0},
    "house_10": {"name": "🏭 Мануфактура", "price": 40000, "income": 2400.0},
    "house_11": {"name": "🏗️ Склад", "price": 90000, "income": 5400.0},
    "house_12": {"name": "🏙️ Бизнес-центр", "price": 200000, "income": 12000.0},
    "house_13": {"name": "🌐 Сеть АЗС", "price": 450000, "income": 27000.0},
    "house_14": {"name": "🏨 Сеть отелей", "price": 1000000, "income": 60000.0},
    "house_15": {"name": "⚡ Электростанция", "price": 2200000, "income": 138000.0},
    "house_16": {"name": "🚢 Морской порт", "price": 5000000, "income": 312000.0},
    "house_17": {"name": "✈️ Аэропорт", "price": 11000000, "income": 720000.0},
    "house_18": {"name": "🎡 Парк аттракционов", "price": 25000000, "income": 1680000.0},
    "house_19": {"name": "🏙️ Мега-небоскреб", "price": 60000000, "income": 3900000.0},
    "house_20": {"name": "🌐 Корпоративный город", "price": 140000000, "income": 9000000.0}
}

# --- ДОНАТ-БИЗНЕСЫ ЗА ЗВЕЗДЫ (ДОХОД ЗА 1 МИНУТУ) ---
STAR_HOUSES = {
    "star_cafe": {"name": "⭐ Звездная Кофейня", "price": 5, "income": 600000.0},
    "star_1": {"name": "💎 Звездная IT-Корпорация", "price": 15, "income": 2100000.0},
    "star_2": {"name": "🚀 Звездный Космопорт", "price": 40, "income": 7200000.0},
    "star_3": {"name": "🛰️ Звездная Спутниковая связь", "price": 100, "income": 27000000.0},
    "star_4": {"name": "🌌 Межгалактический Звездный Банк", "price": 250, "income": 90000000.0}
}

# --- ИНВЕСТИЦИИ (ДОХОД ЗА 1 МИНУТУ) ---
INVESTMENTS = {
    "inv_0": {"name": "☕ Кофейный ларек (Старт)", "price": 10, "income": 18.0},
    "inv_1": {"name": "🌱 Крипто-ферма", "price": 75, "income": 24.0},
    "inv_2": {"name": "☕ Кофейный ларек+", "price": 200, "income": 48.0},
    "inv_3": {"name": "⛽ Автомойка", "price": 750, "income": 120.0},
    "inv_4": {"name": "🚢 Логистическая компания", "price": 3000, "income": 480.0},
    "inv_5": {"name": "✈️ Авиалинии", "price": 12500, "income": 1800.0},
    "inv_6": {"name": "🎬 Киностудия", "price": 50000, "income": 6000.0},
    "inv_7": {"name": "🧬 Биотех-лаборатория", "price": 200000, "income": 21000.0},
    "inv_8": {"name": "⚛️ Квантовый реактор", "price": 1000000, "income": 72000.0},
    "inv_9": {"name": "🪐 Добыча на астероидах", "price": 5000000, "income": 240000.0}
}
...
# Фоновый поток начислений дохода (раз в минуту / 60 секунд)
def income_loop():
    while True:
        time.sleep(60)
        conn = sqlite3.connect("game.db", check_same_thread=False)
...
def main_menu_markup():
    ...
    markup.row(InlineKeyboardButton("⏰ Бонус (3 монеты)", callback_data="hourly_bonus"), InlineKeyboardButton("📢 Каналы", callback_data="channels"))
    return markup

@bot.message_handler(commands=["start"])
def send_welcome(message):
    get_user(message.chat.id)
    bot.send_message(message.chat.id, "🌆 Игра обновлена! Теперь весь пассивный доход начисляется раз в минуту.", reply_markup=main_menu_markup())
...
if __name__ == "__main__":
    print("Бот с начислением дохода за минуту запущен...")
