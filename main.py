...
# --- ОБЫЧНЫЕ БИЗНЕСЫ (ДОХОД УМЕНЬШЕН В 3 РАЗА) ---
HOUSES = {
...
# --- ДОНАТ-БИЗНЕСЫ ЗА ЗВЕЗДЫ (НЕ ТРОГАЛИСЬ, ДАЮТ МАКСИМУМ) ---
STAR_HOUSES = {
...
# --- ИНВЕСТИЦИИ (ДОХОД УМЕНЬШЕН В 3 РАЗА) ---
INVESTMENTS = {
    "inv_0": {"name": "☕ Кофейный ларек (Старт)", "price": 10, "income": round(18.0 / 3, 2)},
    "inv_1": {"name": "🌱 Крипто-ферма", "price": 75, "income": round(24.0 / 3, 2)},
    "inv_2": {"name": "☕ Кофейный ларек+", "price": 200, "income": round(48.0 / 3, 2)},
    "inv_3": {"name": "⛽ Автомойка", "price": 750, "income": round(120.0 / 3, 2)},
    "inv_4": {"name": "🚢 Логистическая компания", "price": 3000, "income": round(480.0 / 3, 2)},
    "inv_5": {"name": "✈️ Авиалинии", "price": 12500, "income": round(1800.0 / 3, 2)},
    "inv_6": {"name": "🎬 Киностудия", "price": 50000, "income": round(6000.0 / 3, 2)},
    "inv_7": {"name": "🧬 Биотех-лаборатория", "price": 200000, "income": round(21000.0 / 3, 2)},
    "inv_8": {"name": "⚛️ Квантовый реактор", "price": 1000000, "income": round(72000.0 / 3, 2)},
    "inv_9": {"name": "🪐 Добыча на астероидах", "price": 5000000, "income": round(240000.0 / 3, 2)}
}
...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, "🌆 Баланс игры обновлен! Доходы бесплатных бизнесов и инвестиций уменьшены в 3 раза.", reply_markup=main_menu_markup())
...
if __name__ == "__main__":
    print("Бот с уменьшенным в 3 раза доходом запущен...")
